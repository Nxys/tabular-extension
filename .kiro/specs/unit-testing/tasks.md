...
- [ ]* 2.2 测试 storage.ts 错误处理
  - 测试 chrome.storage 失败时的内存降级
  - 测试内存存储的一致性
  - 验证错误日志输出
  - _需求：8.1_

- [ ]* 2.3 编写 storage.ts 属性测试
  - **属性 2：存储往返一致性**
...
- [ ]* 2.5 测试 usage.ts 跨天重置
  - 使用 Jest fake timers 控制时间
  - 测试跨天边界的重置逻辑
  - 验证统计数据重置
  - _需求：1.2, 9.2_

- [ ]* 2.6 测试 usage.ts 错误处理
  - 测试 record() 失败时的优雅降级
  - 验证主流程不受影响
  - _需求：8.3_

- [ ]* 2.7 编写 usage.ts 属性测试
  - **属性 1：使用次数单调性**
...
- [ ]* 2.11 测试 index.ts 异常处理
  - 测试消息处理异常时的兜底返回
  - 测试未知 action 类型的处理
  - 验证兜底响应格式
  - _需求：1.7, 8.2_

- [ ]* 2.12 测试 index.ts 使用次数限制
