 ...
 * 等待插件面板出现
 */
export async function waitForPanel(page: Page, panelSelector: string = '.tabular-panel'): Promise<void> {
 ...
 * 检查插件面板是否显示
 */
export async function isPanelVisible(page: Page, panelSelector: string = '.tabular-panel'): Promise<boolean> {
  try {
...
export async function clickPanelButton(page: Page, buttonText: string): Promise<void> {
  const button = page.locator('.tabular-panel').getByText(buttonText);
  await button.click();
}
